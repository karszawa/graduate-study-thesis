...
また、ユーザが記述したアルゴリズムをコンパイルするためにはJDK(Java Development Kit)8が必要となる\cite{jdk}。
...
ここで$N$はプロセスの個数である。
$v$の初期値はプロセス$P_i$では$v[i]=1, v[j]=0(j \neq i)$で、$q$の初期値はプロセスによらず$q=[\infty,\infty,\cdots,\infty]$である。
...
5. $P_i$から解放メッセージを受信したプロセスは、自身の$q[i]$に$\infty$を代入する。
...
そこで、前川のアルゴリズムでは、要求メッセージreqに論理時計による時刻印を付加することで、要求に優先度を付ける。
...
8. $P_j$が$P_i$からreleaseを受信したら、$P_i$によるロックを解除する。待ち行列$QUEUE_j$が空でなければ
	1. 待ち行列$QUEUE_j$にあるreqの中で最古の時刻印を持つreq($TS_h$,$P_h$)を取り出し、プロセス$P_h$にlockedを送る。
